...
resource "aws_sqs_queue" "customers2_queue" {
  name  = "customers2"
  redrive_policy = jsonencode({
                    "deadLetterTargetArn" = aws_sqs_queue.deadletter_customers_queue.arn,
                    "maxReceiveCount" = 3
...
# Creating a policy to sqs queue to allow SNS publish a message into it
resource "aws_sqs_queue_policy" "customers2_queue_policy" {
  queue_url = aws_sqs_queue.customers2_queue.id
  policy = jsonencode(
        {
            "Version": "2012-10-17",
            "Id": "sqspolicy",
            "Statement": [
            {
                ...
                "Action": "sqs:SendMessage",
                "Resource": aws_sqs_queue.customers2_queue.arn,
                "Condition": {
                    "ArnEquals": {
                        "aws:SourceArn": aws_sns_topic.customers_topic.arn
                    }
...
# Creating the subscription between the SNS topic and the SQS queue
